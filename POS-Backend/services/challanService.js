...
const { mapChallan, mapChallanItem } = require('../db/mapper');
...
async function generateChallanNumber(client) {
  ...
  const seqRes = await client.query("SELECT nextval('challan_num_seq') AS seq");
 ...
 */
async function createChallan({ customerId, items, notes, organizationId, createdBy }) {
  if (!customerId) {
  ...
  for (const it of items) {
    ...
    if (seenProductIds.has(pId)) {
      const err = new Error(`Duplicate product (${pId}) in challan is not allowed. Please specify total quantity in a single line.`);
      err.statusCode = 400;
      throw err;
    }
    ...
    if (!Number.isInteger(qty) || qty <= 0) {
      const err = new Error(`Quantity for product ${pId} must be a positive integer`);
      err.statusCode = 400;
      throw err;
    }
  }
  ...
  try {
    ...
    for (const it of items) {
      ...
      if (prodRes.rows.length === 0) {
        const err = new Error(`Product '${pId}' not found in catalogue`);
        err.statusCode = 404;
        throw err;
      }
 ...
 */
async function getAllChallans({ page = 1, limit = 10, status, customerId, search, organizationId }) {
 ...
 */
async function getChallanById(id, organizationId) {
  const challanQuery = `
    SELECT 
      ch.id,
      ch.challan_number,
  ...
  `;
  const challanRes = await pool.query(challanQuery, [id, organizationId]);
 ...
 */
async function updateDraftChallan(id, { customerId, items, notes, organizationId }) {
  if (!customerId) {
    const err = new Error('Customer ID is required');
    err.statusCode = 400;
    throw err;
  ...
  if (!Array.isArray(items) || items.length === 0) {
    const err = new Error('At least one item is required in challan');
    err.statusCode = 400;
    throw err;
  }
  ...
  for (const it of items) {
    ...
    if (seenProductIds.has(pId)) {
      const err = new Error(`Duplicate product (${pId}) in challan is not allowed`);
      err.statusCode = 400;
      throw err;
    }
    seenProductIds.add(pId);
  ...
  try {
    ...
    if (currentStatus !== 'DRAFT') {
      const err = new Error(`Cannot modify challan with status '${currentStatus}'. Only DRAFT challans can be edited.`);
      err.statusCode = 409;
      throw err;
    }
 ...
 */
async function confirmChallan(id, { organizationId, userId }) {
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    ...
    if (currentStatus === 'CONFIRMED') {
      const err = new Error('Sales Challan is already CONFIRMED. Cannot confirm again.');
      err.statusCode = 409;
      throw err;
    }
    if (currentStatus === 'CANCELLED') {
      const err = new Error('Cannot confirm a CANCELLED Sales Challan.');
      err.statusCode = 409;
      throw err;
    }
    if (currentStatus !== 'DRAFT') {
      const err = new Error(`Cannot confirm challan with status '${currentStatus}'.`);
      err.statusCode = 409;
      throw err;
    }
    ...
    if (itemsRes.rows.length === 0) {
      const err = new Error('Sales Challan contains no items to confirm');
      err.statusCode = 400;
      throw err;
    }
    ...
    if (insufficientErrors.length > 0) {
      const err = new Error(`Challan confirmation failed: ${insufficientErrors.join('; ')}`);
      err.statusCode = 409;
 ...
 */
async function cancelChallan(id, { organizationId, userId }) {
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    ...
    if (currentStatus === 'CONFIRMED') {
      const err = new Error('Confirmed challans cannot be cancelled directly.');
      err.statusCode = 409;
      throw err;
    }
    if (currentStatus === 'CANCELLED') {
    ...
    if (currentStatus !== 'DRAFT') {
      const err = new Error(`Cannot cancel challan with status '${currentStatus}'.`);
      err.statusCode = 409;
      throw err;
    }
