/**
 * Customer Service (CRM & Follow-ups)
 * Phase 4 - Mini ERP + CRM Operations Portal
 *
 * Implements customer management, classification (Retail/Wholesale/Distributor),
 * status lifecycle (Lead/Active/Inactive), search, pagination, and append-only follow-ups.
 */

import { query, pool } from '../db/index';
import { mapCustomer, mapCustomerFollowup, Customer, CustomerFollowup } from '../db/mapper';
 ...
 */
function validateCustomerPayload(data: Partial<CreateCustomerData>, isUpdate = false): string[] {
  const errors: string[] = [];
...
export const listCustomers = async ({
  ...
  followUpDate,
  organizationId
}: CustomerListParams = {}): Promise<PaginatedCustomers> => {
  ...
  return {
    customers: dataRes.rows.map(mapCustomer).filter(Boolean) as Customer[],
    pagination: {
      page: pageNum,
      limit: limitNum,
      total,
...
export const getCustomerById = async (
  ...
  if (organizationId && row.organization_id && row.organization_id !== organizationId) {
    const err = new Error('Unauthorized: Customer does not belong to this organization') as Error & { status?: number };
    err.status = 403;
    throw err;
  }
...
export const createCustomer = async (
  ...
  if (validationErrors.length > 0) {
    const err = new Error(validationErrors.join('; ')) as Error & { status?: number };
    err.status = 400;
    throw err;
  }
...
export const updateCustomer = async (
  ...
  const rawMobile = data.mobile || data.phone;
  const mobile = rawMobile !== undefined ? rawMobile.trim() : existing.mobile;
  const phone = mobile;
...
export const deleteOrDeactivateCustomer = async (
  id: string,
  organizationId?: string | null
): Promise<Customer | null> => {
  const existing = await getCustomerById(id, organizationId);
...
export const listFollowups = async (
  ...
  return res.rows.map(mapCustomerFollowup).filter(Boolean) as CustomerFollowup[];
};

export const addFollowup = async (
  customerId: string,
  data: AddFollowupData,
  userId?: string | null,
  organizationId?: string | null
  ...
  if (!followUpDate) {
    const err = new Error('followUpDate is required') as Error & { status?: number };
    err.status = 400;
    throw err;
  }
  ...
  if (!noteText || typeof noteText !== 'string' || !noteText.trim()) {
    const err = new Error('Follow-up notes are required and cannot be empty') as Error & { status?: number };
    err.status = 400;
    throw err;
  }
