 ...
 *
 * Handles store directory management, store user onboarding, and store-specific pricing overrides.
 */

import crypto from 'crypto';
import { pool, query } from '../db/index';
import { mapStore, mapStorePrice, mapProduct, mapUser, Store, StorePrice, Product } from '../db/mapper';
import type { MappedUser } from '../types/auth';
import { generateNextStoreId } from '../utils/storeIdGenerator';
import { sendStoreSignupEmail } from '../utils/emailService';
...
export interface UpsertStorePriceData {
...
export const getAll = async (filter: StoreFilter = {}): Promise<Store[]> => {
  ...
  return res.rows.map(mapStore).filter(Boolean) as Store[];
};
...
export const searchAndFilter = async ({
  ...
  fromDate,
  toDate
}: StoreSearchParams): Promise<Store[]> => {
...
export const createStore = async (storeData: CreateStoreData): Promise<CreateStoreResult> => {
  const email = (storeData.email || '').trim().toLowerCase();
  if (!email) {
    const err = new Error('Email is required') as Error & { httpStatus?: number };
    err.httpStatus = 400;
    throw err;
  }
  ...
  try {
    ...
    if (existingUser && existingUser.status === 'active') {
      const err = new Error('Email already exists') as Error & { httpStatus?: number };
      err.httpStatus = 409;
      throw err;
    }
...
export const update = async (id: string, data: UpdateStoreData): Promise<Store | null> => {
...
export const listStorePrices = async (storeId: string, sku?: string): Promise<StorePrice[]> => {
  ...
  sql += ' ORDER BY created_at DESC';
  const res = await query(sql, params);
  return res.rows.map(mapStorePrice).filter(Boolean) as StorePrice[];
};

export const upsertStorePrice = async (
  storeId: string,
  sku: string,
  data: UpsertStorePriceData
